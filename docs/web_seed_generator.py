...
def randomize_entrances(areas):
    ...
    while i < len(lines):
        ...
        match = re.match(".*<Door name=\"(.+)\".*", lines[i])
        if match:
            name = match.group(1)
            group = int(selectText("<Group>", "</Group>", lines[i+4]))
            x = int(selectText("<X>", "</X>", lines[i+2]))
            y = int(selectText("<Y>", "</Y>", lines[i+3]))
            innerDoors[group].append(Door(name + "InnerDoor", x, y))

            group = int(selectText("<Group>", "</Group>", lines[i+9]))
            x = int(selectText("<X>", "</X>", lines[i+7]))
            y = int(selectText("<Y>", "</Y>", lines[i+8]))
            outerDoors[group].append(Door(name + "OuterDoor", x, y))
...
def placeItemsMulti(seed, expPool, hardMode, includePlants, shardsMode, limitkeysMode, cluesMode, noTeleporters, modes, flags, syncFlags, starvedMode, preferPathDifficulty, setNonProgressiveMapstones, playerCountIn, balanced, entrance):
    ...
    placement = placeItems(seed, expPool, hardMode, includePlants, shardsMode, limitkeysMode, cluesMode, noTeleporters, modes, playerFlags, starvedMode, preferPathDifficulty, setNonProgressiveMapstones, playerCountIn, playerID, balanced, entrance)
    if not placement:
        return placeItemsMulti(seed, expPool, hardMode, includePlants, shardsMode, limitkeysMode, cluesMode, noTeleporters, modes, flags, syncFlags, starvedMode, preferPathDifficulty, setNonProgressiveMapstones, playerCountIn, balanced, entrance)
...
def placeItems(seed, expPool, hardMode, includePlants, shardsMode, limitkeysMode, cluesMode, noTeleporters, modes, flags, starvedMode, preferPathDifficulty, setNonProgressiveMapstones, playerCountIn, playerIDIn, balancedIn, entrance, depth=0):
    ...
    while i < len(lines):
        ...
        if match:
            match2 = re.match(".*<Entrance/>.*", lines[i+1])
            if match2:
                i += 1
                if entrance:
                    continue
            ...
            if not assignQueue:
                ...
                if not reservedLocations:
                    ...
                    if depth > playerCount * playerCount:
                        return
                    return placeItems(seed, expPool, hardMode, includePlants, shardsMode, limitkeysMode, cluesMode, noTeleporters, modes, flags, starvedMode, preferPathDifficulty, setNonProgressiveMapstones, playerCount, playerID, balanced, entrance, depth+1)
...
class LogicListener():
    ...
    def onChange(self, sender):
        ...
        pathSpeedLure.setChecked(False)
...
def generate():
    ...
    if seed.strip().lower() == "daily":
        ...
        seed = str(now.year) + "-" + str(now.month) + "-" + str(now.day)
    ...
    balanced = algSelection.getSelectedIndex() == 1
    entrance = entranceBox.isChecked()
    ...
    if enableSync.isChecked():
        if shareSkills.isChecked():
    ...
    playerCountIn = 1
    if enableSync.isChecked():
        playerCountIn = playerCountSelection.getSelectedIndex()+1
        sharedSeedFlags = []
        if shareSkills.isChecked():
        ...
        syncFlags += ",shared=" + "+".join(sharedSeedFlags)
        syncFlags += ",mode=" + shareModeSelection.getItemText(shareModeSelection.getSelectedIndex())
        sync_id = syncIDSelection.getText()
        if not sync_id:
...
def download_seed():
    ...
    if placements:
        ...
        element.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(placements[downloadPlayerID][0]));
        ...
        downloadPlayerID += 1
        downloadPlayerID %= len(placements)

        if len(placements) > 1:
            seedButton.setText("Download Seed (P" + str(downloadPlayerID+1) + ")")

def download_spoiler():
    ...
    if placements:
        ...
        element = DOM.createElement('a')
        element.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(placements[downloadSpoilerID][1]));
        ...
        downloadSpoilerID += 1
        downloadSpoilerID %= len(placements)

        if len(placements) > 1:
            spoilerButton.setText("Download Spoiler (P" + str(downloadSpoilerID+1) + ")")
...
def main():
    ...
    row4 = DisclosurePanel("Sync Options", StyleName="row")
    row5 = HorizontalPanel(HorizontalAlignment=HasAlignment.ALIGN_LEFT, StyleName="row")
    panel.add(row0)
    panel.add(row1)
    panel.add(row2)
    panel.add(row3)
    ...
    title = HTML("Ori DE Randomizer (v2.6.0)", StyleName="title")
    ...
    entranceBox = CheckBox("Entrance Shuffle")
    ...
    global pathSpeedLure
    pathSpeedLure = CheckBox("Speed-Lure")
    pathSpeedLure.addClickListener(customPathListener)
    paths2.add(pathSpeedLure)
    ...
    paths4.add(pathCDashFarming)

    #row 4 - sync seed options

    syncPanel = VerticalPanel()

    syncShare = HorizontalPanel(StyleName="row")
    syncVariables = HorizontalPanel(StyleName="row")

    syncPanel.add(syncShare)
    syncPanel.add(syncVariables)

    global enableSync
    enableSync = CheckBox("Enable Sync")
    syncShare.add(enableSync)

    global shareSkills
    shareSkills = CheckBox("Skills")
    shareSkills.setChecked(True)
    syncShare.add(shareSkills)

    global shareKeys
    shareKeys = CheckBox("Keys")
    shareKeys.setChecked(True)
    syncShare.add(shareKeys)

    global shareEvents
    shareEvents = CheckBox("Events")
    syncShare.add(shareEvents)

    global shareUpgrades
    shareUpgrades = CheckBox("Upgrades")
    syncShare.add(shareUpgrades)

    global shareTeleporters
    shareTeleporters = CheckBox("Teleporters")
    syncShare.add(shareTeleporters)

    playerCountPanel = HorizontalPanel(StyleName="inner_row")

    playerCountText = HTML("Player Count", StyleName="label")
    playerCountPanel.add(playerCountText)

    global playerCountSelection
    playerCountSelection = ListBox(VisibleItemCount=1, StyleName="dropdown_small")
    for count in range(1,9):
        playerCountSelection.addItem(str(count))
    playerCountSelection.setSelectedIndex(1)
    playerCountPanel.add(playerCountSelection)

    syncVariables.add(playerCountPanel)

    shareModePanel = HorizontalPanel(StyleName="inner_row")

    shareModeText = HTML("Share Mode", StyleName="label")
    shareModePanel.add(shareModeText)

    global shareModeSelection
    shareModeSelection = ListBox(VisibleItemCount=1, StyleName="dropdown")
    shareModeSelection.addItem("Shared")
    shareModeSelection.addItem("Swap")
    shareModeSelection.addItem("Split")
    shareModeSelection.addItem("None")
    shareModePanel.add(shareModeSelection)

    syncVariables.add(shareModePanel)

    syncIDPanel = HorizontalPanel(StyleName="inner_row")

    syncIDText = HTML("Sync ID", StyleName="label")
    syncIDPanel.add(syncIDText)

    global syncIDSelection
    syncIDSelection = TextBox(StyleName="seed", MaxLength=10)
    syncIDPanel.add(syncIDSelection)
    ...
    seedPanel = VerticalPanel()

    seedInputPanel = HorizontalPanel(StyleName="inner_row")
    ...
    seedInputPanel.add(seedSelection)

    algInputPanel = HorizontalPanel(StyleName="inner_row")

    algText = HTML("Fill Algorithm", StyleName="label")
    global algSelection
    algSelection = ListBox(VisibleItemCount=1, StyleName="dropdown")
    algSelection.addItem("Classic")
    algSelection.addItem("Balanced")
    algSelection.setSelectedIndex(1);

    algInputPanel.add(algText)
    algInputPanel.add(algSelection)

    seedPanel.add(seedInputPanel)
    seedPanel.add(algInputPanel)

    buttonPanel = VerticalPanel()

    global genButton
    global placements
    placements = []
    genButton = Button("Generate", generate, StyleName='gen_button')

    buttonPanel.add(genButton)

    downloadPanel = HorizontalPanel()

    global spoilerButton
    spoilerButton = Button("Download Spoiler", download_spoiler, StyleName="inactive_button")

    global seedButton
    seedButton = Button("Download Seed", download_seed, StyleName="inactive_button")

    downloadPanel.add(spoilerButton)
    downloadPanel.add(seedButton)
