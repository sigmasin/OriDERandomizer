...
$pyjs.loaded_modules['web_seed_generator'] = function (__mod_name__) {
	...
	$m['assign_random'] = function(recurseCount) {
		...
		var $add76,$iter20_iter,$iter20_nextval,$iter20_type,$add77,$and6,$div11,value,$and5,$iter20_idx,$div12,key,position,$and7,$add78,$add75,$iter20_array;
	...
	$m['placeItems'] = function(seed, expPool, hardMode, includePlants, shardsMode, limitkeysMode, noTeleporters, doLocationAnalysis, doSkillOrderAnalysis, modes, flags, starvedMode, preferPathDifficulty, setNonProgressiveMapstones) {
		...
		if ($p['bool']($m['limitkeys'])) {
			...
			while ($p['bool'](!$p['bool'](satisfied))) {
				...
				if ($p['bool'](($p['bool']($and10=!$p['op_eq'](ginso, forlorn))?($p['bool']($and11=!$p['op_eq'](ginso, horu))?($p['bool']($and12=!$p['op_eq'](forlorn, horu))?($p['cmp']($p['__op_add']($add169=ginso,$add170=forlorn), 26) == -1):$and12):$and11):$and10))) {
					satisfied = true;
				}
			}
	...
	, 1, [null,null,['self'],['element']]);
		...
		$method = $pyjs__bind_method2('onChange', function(sender) {
			...
			(typeof generate == "undefined"?$m.generate:generate)();
			return null;
		}
	...
	$m['generate'] = function() {
		var limitkeys,prefer_path_difficulty,loc_analysis,placement,modes,non_progressive_mapstones,includePlants,hard,shards,analysis,element,$add232,exp_pool,seed,flags,no_teleporters,starved,test,$add231,mode;
		seed = 1;
		exp_pool = 10000;
		hard = true;
		includePlants = true;
		...
		element['setAttribute']('href', $p['__op_add']($add231='data:text/plain;charset=utf-8,',$add232=(typeof encodeURIComponent == "undefined"?$m.encodeURIComponent:encodeURIComponent)(placement.__getitem__(0))));
	...
	$m['main'] = function() {
		var pathExtreme,modeText,variations1,variations3,variations2,noPlantsBox,logicText,pathTimedLevel,paths3,hardBox,pathGlitched,row1_2,row1_3,title,pathNormal,pathSpeed,pathCDash,pathDBoostLight,seedPanel,variationPanel,paths5,diffSelection,pathExtended,noBonusBox,diffText,seedSelection,logicSelection,modeSelection,logicListener,row1_1,paths2,pathsPanel,paths1,paths4,panel,row0,row1,row2,row3,row4,starvedBox,pathDBash,variationText,seedText,genButton,pathLureHard,pathDBoostHard,zeroBox,pathExtendedDamage,forceBox,ohkoBox,pathLure,noTeleBox,pathDBoost,pathCDashFarming,nonProgMapBox;
		...
		logicSelection = $pyjs_kwargs_call(null, $m['ListBox'], null, null, [{VisibleItemCount:1, StyleName:'dropdown'}]);
		...
		logicListener = $m['LogicListener'](logicSelection);
		row1_1['add'](logicText);
		row1_1['add'](logicSelection);
		modeText = $pyjs_kwargs_call(null, $m['HTML'], null, null, [{StyleName:'label'}, 'Mode']);
		modeSelection = $pyjs_kwargs_call(null, $m['ListBox'], null, null, [{VisibleItemCount:1, StyleName:'dropdown'}]);
		...
		diffSelection = $pyjs_kwargs_call(null, $m['ListBox'], null, null, [{VisibleItemCount:1, StyleName:'dropdown'}]);
		...
		forceBox = $m['CheckBox']('Force Trees');
		hardBox = $m['CheckBox']('Hard Mode');
		noTeleBox = $m['CheckBox']('No Teleporters');
		starvedBox = $m['CheckBox']('Starved');
		ohkoBox = $m['CheckBox']('OHKO');
		noPlantsBox = $m['CheckBox']('No Plants');
		nonProgMapBox = $m['CheckBox']('Discrete Mapstones');
		zeroBox = $m['CheckBox']('0 XP');
		noBonusBox = $m['CheckBox']('No Bonuses');
		...
		pathNormal = $m['CheckBox']('Normal');
		pathLure = $m['CheckBox']('Lure');
		pathExtended = $m['CheckBox']('Extended');
		paths1['add'](pathNormal);
		paths1['add'](pathLure);
		paths1['add'](pathExtended);
		pathSpeed = $m['CheckBox']('Speed');
		pathLureHard = $m['CheckBox']('Lure-Hard');
		pathExtendedDamage = $m['CheckBox']('Extended-Damage');
		paths2['add'](pathSpeed);
		paths2['add'](pathLureHard);
		paths2['add'](pathExtendedDamage);
		pathDBoostLight = $m['CheckBox']('DBoost-Light');
		pathDBash = $m['CheckBox']('DBash');
		pathExtreme = $m['CheckBox']('Extreme');
		paths3['add'](pathDBoostLight);
		paths3['add'](pathDBash);
		paths3['add'](pathExtreme);
		pathDBoost = $m['CheckBox']('DBoost');
		pathCDash = $m['CheckBox']('CDash');
		pathTimedLevel = $m['CheckBox']('Timed-Level');
		paths4['add'](pathDBoost);
		paths4['add'](pathCDash);
		paths4['add'](pathTimedLevel);
		pathDBoostHard = $m['CheckBox']('DBoost-Hard');
		pathCDashFarming = $m['CheckBox']('CDash-Farming');
		pathGlitched = $m['CheckBox']('Glitched');
		...
		seedSelection = $pyjs_kwargs_call(null, $m['TextBox'], null, null, [{StyleName:'seed', MaxLength:10}]);
		genButton = $pyjs_kwargs_call(null, $m['Button'], null, null, [{StyleName:'button'}, 'Generate', $m['generate']]);
