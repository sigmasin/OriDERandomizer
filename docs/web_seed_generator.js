...
$pyjs.loaded_modules['web_seed_generator'] = function (__mod_name__) {
		...
		while (typeof($p['__wrapped_next']($iter9_nextval).$nextval) != 'undefined') {
			...
			if ($p['bool']($m.areasReached['keys']().__contains__(area))) {
				...
				while (typeof($p['__wrapped_next']($iter10_nextval).$nextval) != 'undefined') {
					...
					if ($p['bool']($p['op_eq']($p['getattr'](location, 'zone'), 'Forlorn'))) {
						$m['inForlorn'] = true;
					}
				}
	...
	$m['assign_random'] = function(recurseCount) {
		...
		var $iter20_nextval,$and9,$add80,$add78,$iter20_array,$div11,$div12,$and8,$iter20_iter,$or7,$or6,$or9,$or8,$and6,$and7,$and10,$and11,key,$add77,$iter20_type,value,$add79,$iter20_idx,position;
		...
		while (typeof($p['__wrapped_next']($iter20_nextval).$nextval) != 'undefined') {
			...
			if ($p['bool'](($p['cmp'](value, position) < 1))) {
				if ($p['bool'](($p['bool']($or6=($p['bool']($and6=(typeof starved == "undefined"?$m.starved:starved))?($p['bool']($and7=(typeof skillsOutput == "undefined"?$m.skillsOutput:skillsOutput).__contains__(key))?($p['cmp'](recurseCount, 3) == -1):$and7):$and6))?$or6:($p['bool']($and9=$m['inForlorn'])?($p['bool']($and10=($p['cmp'](recurseCount, 5) == -1))?($p['bool']($or8=$p['op_eq'](key, 'GumonSealShard'))?$or8:$p['op_eq'](key, 'ForlornKey')):$and10):$and9)))) {
	...
	$m['assign'] = function(item) {
		var $or14,$sub13,$sub12,$sub11,$sub10,$sub14,$or15,$sub9,$or11,$or10,$or13,$or12,$add82,$add81;
		...
		if ($p['bool'](($p['bool']($or10=$p['op_eq'](item, 'EC'))?$or10:($p['bool']($or11=$p['op_eq'](item, 'KS'))?$or11:$p['op_eq'](item, 'HC'))))) {
		...
		else if ($p['bool'](($p['bool']($or13=$p['op_eq'](item, 'WaterVeinShard'))?$or13:($p['bool']($or14=$p['op_eq'](item, 'GumonSealShard'))?$or14:$p['op_eq'](item, 'SunstoneShard'))))) {
			if ($p['bool'](($p['cmp']((typeof costs == "undefined"?$m.costs:costs).__getitem__(item), 0) == 1))) {
	...
	$m['assign_to_location'] = function(item, location) {
		var $add134,$add127,$add89,$add88,$add129,$add131,$add116,$add100,$add83,$add117,$add114,$add115,$add87,$add86,$add85,$add84,$add133,$add108,$sub18,$add121,$add105,$add123,$add113,$add104,$add128,$add118,$add132,$add119,$add124,$add136,$add94,$add120,$sub17,$add95,$add137,$add110,$mul22,$mul21,$add101,$add103,$add102,$add98,$add99,$add107,$add106,assignment,$add96,$add97,$add90,$add91,$add92,$add93,$and12,$and13,$sub16,$add111,$add138,$add126,$add140,$add130,$add135,value,$sub15,$add109,$add122,$add125,$add139,$add112;
		assignment = '';
		if ($p['bool'](($p['bool']($and12=!$p['bool']((typeof nonProgressiveMapstones == "undefined"?$m.nonProgressiveMapstones:nonProgressiveMapstones)))?$p['op_eq']($p['getattr'](location, 'orig'), 'MapStone'):$and12))) {
	...
	$m['placeItems'] = function(seed, expPool, hardMode, includePlants, shardsMode, limitkeysMode, cluesMode, noTeleporters, modes, flags, starvedMode, preferPathDifficulty, setNonProgressiveMapstones) {
		...
		if ($p['bool']($m['limitkeys'])) {
			...
			while ($p['bool'](!$p['bool'](satisfied))) {
				...
				if ($p['bool'](($p['bool']($and14=!$p['op_eq'](ginso, forlorn))?($p['bool']($and15=!$p['op_eq'](ginso, horu))?($p['bool']($and16=!$p['op_eq'](forlorn, horu))?($p['cmp']($p['__op_add']($add161=ginso,$add162=forlorn), 26) == -1):$and16):$and15):$and14))) {
					satisfied = true;
				}
			}
		...
		while ($p['bool'](($p['cmp'](i, $p['len'](lines)) == -1))) {
			...
			if ($p['bool'](($p['bool']($and18=match)?currentConnection:$and18))) {
			...
			if ($p['bool'](($p['bool']($and20=match)?currentConnection:$and20))) {
		...
		while ($p['bool'](($p['cmp']($m['itemCount'], 0) == 1))) {
			...
			if ($p['bool'](($p['bool']($and22=!$p['bool']($m['doorQueue']))?!$p['bool']($m['mapQueue']):$and22))) {
